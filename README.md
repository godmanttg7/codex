...
- 报修工单创建、指派、状态更新、取消、删除都会触发设施状态重算，并写入站内消息。
...
- 公厕基础信息管理：新增/查看公厕档案。
- 设施状态管理：展示设施状态；`tb_repair_order.status` 是维修状态唯一真相源，`tb_facility.status` 是由全部未结束工单重算出的展示缓存。
- 设施报修工单：创建工单、指派工单、状态流转（PENDING/REPAIRING/CHECKING/FINISHED/CANCELED）、取消工单、删除工单。
- 耗材预警看板：查看低库存预警。
...
- `PATCH /api/manage/repairs/{id}/assign`
- `PATCH /api/manage/repairs/{id}/status`
- `PATCH /api/manage/repairs/{id}/cancel`
- `DELETE /api/manage/repairs/{id}`
- `GET /api/manage/consumables`
- `GET /api/manage/messages`

## 设施状态计算规则
- 维修状态唯一真相源：`tb_repair_order.status`。
- `tb_facility.status` 仅作为列表展示和统计缓存，每次创建、指派、更新、取消、删除工单后通过 `recomputeFacilityStatus(facilityId)` 重算。
- 重算优先级：存在 `REPAIRING`/`CHECKING` 工单 => `REPAIR`；否则存在 `PENDING` 工单 => `FAULT`；否则 => `NORMAL`。
- 因此同一设施同时存在 `A=REPAIRING`、`B=PENDING` 时，最终设施状态为 `REPAIR`，不会被某个已完成/已删除工单直接写成 `NORMAL`。
